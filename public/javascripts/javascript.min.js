...
/* Additional.js */
function helpify(){$(".btn-primary").click(function(){var e=$(this);e.button("loading")});$("img").on().tooltip();$(".admintooltip").on().tooltip()}function showCredits(){$("#modalCredits").modal("show")}$("#ajaxplayday").hide();$("#ajaxloader").hide();$("#ajaxusers").hide();$(document).on("DOMNodeInserted",function(e){helpify()});$(document).ready(function(){$(".alert-info").delay(4e3).slideToggle();$(".alert-success").delay(4e3).slideToggle();helpify();window.addEventListener("load",function(){setTimeout(function(){window.scrollTo(0,1)},0)});if($(".usermanagement").length!=0){$(".usermanagement").dataTable({bPaginate:false,bInfo:false,aaSorting:[[1,"asc"]],aoColumnDefs:[{bSortable:false,aTargets:[0,4,5,6]}],oLanguage:{sSearch:'<i class="icon-search"></i>'}})}if($(".standings").length!=0){$(".standings").dataTable({bPaginate:false,bInfo:false,aaSorting:[[0,"asc"]],aoColumnDefs:[{bSortable:false,aTargets:[1]}],oLanguage:{sSearch:'<i class="icon-search"></i>'}})}$("#fat-btn-loading").click(function(){$.blockUI({message:'<h2>Calculating points...<br/>This may take a moment!<h2><img src="/public/img/ajax-loader.gif" />'});setTimeout($.unblockUI,6e4)})});$(".navbar a, .subnav a").smoothScroll();(function(e){e(function(){function a(){var e,i=t.scrollTop();if(i>=o&&!isFixed){isFixed=1;r.addClass("subnav-fixed");n.css("margin-top",u+s+"px")}else if(i<=o&&isFixed){isFixed=0;r.removeClass("subnav-fixed");n.css("margin-top",u+"px")}}var t=e(window),n=e("body"),r=e(".subnav"),i=e(".navbar").first().height(),s=e(".subnav").first().height(),o=e(".subnav").length&&e(".subnav").offset().top-i,u=parseInt(n.css("margin-top"),10);isFixed=0;a();t.on("scroll",a)})})(window.jQuery);$(window).data("ajaxready",true).scroll(function(e){if($(window).data("ajaxready")==false)return;if($(window).scrollTop()>=$(document).height()-$(window).height()){$(window).data("ajaxready",false);var t=$("#lazyTable tr").length;var n=$("#ajaxusers").text();if(t<n){$("#ajaxloader").show();var r=$("#ajaxplayday").text();$.ajax({cache:false,url:"/overview/playday/"+r+"/"+t,success:function(e){if(e){$("#lazyTable tr:last").after(e)}$(window).data("ajaxready",true)}});$("#ajaxloader").hide()}}})
